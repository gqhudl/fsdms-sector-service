  ...
  stages {
    ...
    stage('docker build & push image on build docker/build server') {
      ...
      steps {
        ...
        script {
          ...
          if(REMOVE_FLAG){
            // sh 'docker container ls -aq --format {{.ID}} --filter name=fsdms-sector-ctn | xargs docker container rm -f'
            // sh 'docker container rm -f $(docker container ls -aq --format {{.ID}} --filter name=.*fsdms-sector-ctn.*)'
        ...
        script {
          ...
          echo "REMOVE_FLAG: ${REMOVE_FLAG}"
          if(REMOVE_FLAG){
            // sh 'docker image ls --format {{.ID}} *${DOCKHUB_USERNAME}/fsdms-sector-srv* | xargs docker image rm -f'
            // sh 'docker image rm -f $(docker image ls --format {{.ID}} *${DOCKHUB_USERNAME}/fsdms-sector-srv*)'
        ...
        // script {
          ...
          //   def customImage = docker.build("${DOCKHUB_USERNAME}/fsdms-sector-srv:latest", '-f Dockerfile .')
    ...
    stage('docker pull image and docker run image on docker/deploy server') {
      ...
      steps {
        ...
        // sh 'docker pull ${DOCKHUB_USERNAME}/fsdms-sector-srv'

        // docker run images
        sh 'docker run -d -p 9053:9053 -v fsdms-data:/fsdms-data --network fsdms-net --name fsdms-sector-ctn ${DOCKHUB_USERNAME}/fsdms-sector-srv -Xms128m -Xmx128m'
      }
    }
